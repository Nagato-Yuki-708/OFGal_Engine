...
LevelViewer::LevelViewer(int length, int width)
    : m_hSharedMem(NULL)
    , m_pSharedMemView(nullptr)
    , m_hEventLevelChanged(NULL)
    , m_hEventRenderPreview(NULL)
{
    oriCanvasSize.x = length;
    oriCanvasSize.y = width;
...
void LevelViewer::Run() {
    ...
    while (true) {
        ...
        if (shouldRender) {
            shouldRender = false;
            ClearScreen();
            RenderingSystem::getInstance().RenderThumbnailAndPrint(m_currentLevel, oriCanvasSize);
        }
    }
}
